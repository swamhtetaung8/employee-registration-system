...
<div class=" min-vh-100 w-100 d-flex justify-content-center align-items-center">
    <div class="bg-white border rounded-4 px-5 py-4 shadow-sm">
        ...
        <form id="loginForm" action="{{ route('auth.check') }}" method="POST" class="my-4">
            @csrf
            <div class="my-3">
                <input autofocus type="text" name="employee_id" placeholder="Employee ID" class="form-control">
            </div>
            <div class="input-group">
                <input id="password" placeholder="Password" type="password" name="password" class="form-control" id="password">
                <button id="showPasswordButton" type="button" class="bi bi-eye-fill input-group-text"></button>
            </div>
        </form>
        <div class=" d-flex justify-content-center">
            <button form="loginForm" type="submit" class="btn btn-primary px-4 py-2">Login</button>
        </div>
    </div>
</div>
@section('script')
